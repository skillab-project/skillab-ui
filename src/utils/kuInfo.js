// Shared Knowledge Unit (KU) metadata: display names and detailed descriptions.
// Used by every view that shows KUs (heatmap, organization skills chart, KU risk)
// so the hover information stays identical across the app.
...
// Helper: "K8" -> "K8 \u2013 Generics and Collections" (falls back to the raw id)
export const getKuLabel = (rawId) => {
    const key = normalizeKuId(rawId);
    return KU_NAMES[key] ? `${key} \u2013 ${KU_NAMES[key]}` : String(rawId);
};

// Helper: description text for a KU id, or "" when unknown
export const getKuDescription = (rawId) => KU_DESCRIPTIONS[normalizeKuId(rawId)] || "";
