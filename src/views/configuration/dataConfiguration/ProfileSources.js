...
const ProfileSources = () => {
    ...
    const fetchProfileCount = async (source) => {
        try {
          ...
          setProfileCounts((prev) => ({ ...prev, [source]: response.data.count }));
            ...
            setProfileCounts((prev) => ({ ...prev, [source]: "Error" }));
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axios.post("http://localhost/api/profiles/sources/new", formData, {
            ...
            <CardBody>
                <Table striped>
                    ...
                    {sources.map((source, index) => (
                        ...
                        <td>{source}</td>
                        <td>{profileCounts[source] ?? "Loading..."}</td>
                        <td className="text-center">
                ...
                <Modal isOpen={isModalOpen} toggle={() => setIsModalOpen(false)}>
                    ...
                    <h2 className="modal-title">Import Profile Source</h2>
