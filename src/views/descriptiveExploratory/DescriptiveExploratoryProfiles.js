...
const DescriptiveExploratoryProfiles = ({filters}) => {
    ...
    const checkLoadedDataOfUser = async () => {
        await axios
                .get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/get_data?user_id=" +userId+ "&session_id=profiles&attribute=data_query_info&storage_name=none")
                .then(async (res) => {
                    console.log("get_data: "+res.data);
                    if (Array.isArray(res.data)) {
                        console.log("Data will be loaded...");
                        await axios
                                .get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/load_data?user_id=" +userId+ "&session_id=profiles&url="+process.env.REACT_APP_API_URL_TRACKER+"%2Fapi%2Fprofiles&body=source%3Dstack-stackoverflow&limit_data_no=5000")
                                .then((res2) => {
    ...
    const fetchDataOccupations = async () => {
        try {
            ...
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/get_data?user_id=" +userId+ "&session_id=profiles&attribute=all_stats&storage_name=skills");

            // Check if response data is empty
            if (Object.keys(response.data).length === 0 && response.data.constructor === Object) {
                console.log('Response get_data for skills, is empty');
                const analyticsResponse = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/analytics_descriptive?user_id=" +userId+ "&session_id=profiles&storage_name=skills&features_query=skills");
    ...
    const fetchLocationData = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/get_data?user_id=" +userId+ "&session_id=profiles&attribute=all_stats&storage_name=location");
            ...
            if (Object.keys(response.data).length === 0 && response.data.constructor === Object) {
                ...
                const analyticsResponse = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/analytics_descriptive?user_id=" +userId+ "&session_id=profiles&storage_name=location&features_query=location");
    ...
    const fetchDataExploratory = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/get_data?user_id=" +userId+ "&session_id=profiles&attribute=explor_stats&storage_name=occupation");
            ...
            if (Object.keys(response.data).length === 0 && response.data.constructor === Object) {
                ...
                // Fetch analytics data if the initial response is empty
                const analyticsResponse = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/analytics_exploratory?user_id=" +userId+ "&session_id=profiles&storage_name=occupation&features_query=skills;;location");
    ...
    const fetchDataTrending = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/get_data?user_id=" +userId+ "&session_id=profiles&attribute=trend_anal&storage_name=trending");
            ...
            if (Object.keys(response.data).length === 0 && response.data.constructor === Object) {
                ...
                const analyticsResponse = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/trend_analysis?user_id=" +userId+ "&session_id=profiles&storage_name=trending&date_field=upload_date&features_query=location&date_format=%25Y-%25m-%25d&what=month");
    ...
    const fetchDataClustering = async (noClustNow) => {
        try {
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/get_data?user_id="+userId+"&session_id=profiles&attribute=skill_clust&storage_name=skillcluster");
            ...
            if (Object.keys(response.data).length === 0 && response.data.constructor === Object) {
                ...
                const analyticsResponse = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/skillcluster?type_now=kmeans&user_id=" +userId+ "&session_id=profiles&storage_name=skillcluster&weight_now=ii_weight&no_clust_now=" +noClustNow+ "&threshold=0.1&umap_nn=5&umap_dim=2&vectors_type=weighting");
