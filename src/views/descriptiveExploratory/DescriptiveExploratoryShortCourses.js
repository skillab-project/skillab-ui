...
const DescriptiveExploratoryShortCourses = ({filters}) => {
    ...
    const fetchLocationData = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND + "/get_data?user_id=" +userId+ "&session_id=courses&attribute=all_stats&storage_name=source");
    ...
    const fetchDataExploratory = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/get_data?user_id=" +userId+ "&session_id=courses&attribute=explor_stats&storage_name=occupation");
    ...
    const fetchDataTrending = async () => {
        try{
            //  check first in getdata before make new analysis
            const response = await axios.get(process.env.REACT_APP_API_URL_LABOUR_DEMAND+"/get_data?user_id=" +userId+ "&session_id=courses&attribute=trend_anal&storage_name=trending");
